...
def two_factor(request):
    if 'authorized_user' in request.session:
        ...
        try:
            ...
            safenetUser = user.safenet_user
            if not safenet.authenticate(safenetUser, password):
                return response_login_not_found(request)
            else:
                request.session['first_factor'] = email
...
def client(request):
    if 'authorized_user' in request.session:
        ...
        safenet = SafeNet()
        try:
            email = request.session['first_factor']
            password = request.POST['password']
        except KeyError:
            return render(request, 'login.html', {})
        ...
        try:
            ...
            del request.session['first_factor']
            safenetUser = user.safenet_user
            if not safenet.authenticate(safenetUser, password):
                return response_login_not_found(request)
            else:
...
def client_send(request):
    ...
    try:
        ...
        for attached in attachment:
            ...
            fa.file_sent_date = timezone.now()
